...
## Corpus Check
- 69 files · ~95,719 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1016 nodes · 1485 edges · 160 communities (57 shown, 103 thin omitted)
...
## Communities (160 total, 103 thin omitted)
...
Cohesion: 0.10
Nodes (27): _build_config(), _default_accel_to_depth(), _depth_to_color_extrinsic(), _drain_accel(), _encode_jpeg(), _grid_to_depth_extrinsic(), main(), parse_imu_axes() (+19 more)
...
Cohesion: 0.15
Nodes (8): BackgroundSubtractor, Begin averaging `frames` frames into a new background plate. Disables         s, Accumulate one frame during capture. Returns True when the plate is         fin, Boolean (H,W) mask: True = keep (closer than background, or background, Background-plate subtraction tests (headless; numpy required).  Run: python3 -, test_capture_and_subtract(), test_clear_disables(), test_margin_and_unknown()
...
Cohesion: 0.06
Nodes (41): aligned_color_grid(), build_message(), _build_message_v2(), build_message_v3(), compute_ray_table(), default_intrinsics(), extract_depth_grid(), gravity_to_view() (+33 more)

### Community 20 - "MoveNet Pose Estimator"
Cohesion: 0.15
Nodes (13): _bench(), decode_movenet(), letterbox(), 2D human pose on the node's color image -> CPOS keypoints (docs/skeleton_pose.md, Depth (metres) under a keypoint: median of the non-zero values in a     (2*half, (H, W, 3) uint8 RGB -> [(joint_id, u, v, conf)] in image pixels.         Per-st, Resize an (H, W, 3) image into a (size, size, 3) letterboxed square     (neares, Standalone model benchmark — the model's raw speed on THIS machine with     no (+5 more)

### Community 21 - "Skeleton Solve Tests"
Cohesion: 0.12
Nodes (22): Solve every sensor's rigid transform into the reference sensor's frame     from, solve_skeleton(), parse_pose(), A synthetic person (world frame, metres): pelvis wanders slowly through     the, yaw_deg,x,y,z[,pitch_deg]' -> (R (3x3 rows), t) — this sensor's     view->world, skeleton_world_joints(), Read one node->central message, dispatching on the leading magic.      Returns, read_message() (+14 more)
...
Cohesion: 0.24
Nodes (7): MoveNetEstimator, Single-person MoveNet via onnxruntime. Tolerant of the common export     varian, object, The real estimator path (onnxruntime session, dtype/layout detection,     lette, Torso confidence follows a per-call script (chair flicker etc.)., _ScriptedEstimator, test_movenet_estimator()
...
Cohesion: 0.09
Nodes (27): Frame message (TCP, little-endian) — see `protocol/frame.py`, Intrinsics handshake (TCP, node → central) — `protocol/frame.py`, Take on disk — see `central/recorder.py`, Wire protocol & take format, frame_to_mesh(), load_calib(), main(), mesh_frame() (+19 more)
...
Cohesion: 0.43
Nodes (6): crypt-capture wire protocol — a single synchronized depth+color frame.  One me, Read exactly n bytes from a blocking socket, or b'' on clean EOF., Read one Frame from a socket, or None on clean connection close., read_frame(), _recv_exactly(), socket

### Community 157 - "object"
Cohesion: 0.33
Nodes (3): _FakeSock, grid->depth extrinsic registration tests (headless; numpy for the relay parts)., test_extrinsic_roundtrip()
...
Cohesion: 0.50
Nodes (4): Feed the threaded worker exactly ONE frame and wait for its inference     (+ a, Isolated confident frames (furniture flukes) never acquire; a person     needs, _step(), test_pose_worker_gate_hysteresis()
...
- **Why does `PreviewServer` connect `Preview Relay Server` to `Pose Model Tests`, `Temporal Depth Denoise`, `Joint Tracker`, `WebSocket & Client Tools`, `Spatial Depth Denoise`, `Frame Protocol & Relay Ingest`, `Recording HTTP Tests`, `Relay Unprojection Helpers`, `Skeleton Solve Tests`, `Pose Worker Thread`, `test_relay_workers.py`?**
