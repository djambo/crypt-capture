...
| `{"type":"skeleton","sensor":"fused","joints":{…},"n":<sensors>}` | the **cross-sensor FUSED skeleton** (docs/skeleton_pose.md, `central/skeleton_fusion.py`) — per-joint robust confidence-weighted merge of the fresh per-sensor skeletons, emitted alongside them on every per-sensor update. `n` = cameras that contributed. Multi-sensor fusion only engages once a rig calibration registers the sensors into one frame (single camera passes through regardless); on a multi-camera rig with NO calibration this message is absent — viewers fall back to the per-sensor skeletons. Same `joints` layout as the per-sensor message; world frame. |
